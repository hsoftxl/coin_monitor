...
- ✅ **多平台实时监控**: 默认针对 Binance 合约 (可扩展支持 OKX/Bybit)
- ✅ **双轨信号 (Dual-Track)**: 1m 主力拉升 + 15m 稳步上涨
- ✅ **智能信号识别**: 自动检测全球协同、机构吸筹、诱多/出货等信号
- ✅ **合约/现货切换**: 支持一键切换 Binance 合约 (Futures) 或现货 (Spot) 监控
- ✅ **只做多 (Long Only)**: 屏蔽所有出货/做空信号，专注于上涨机会
- ✅ **多周期趋势确认**: 结合 5m/15m/1h 趋势过滤假信号
...
编辑 `src/config.py`:
```python
    # 市场类型: 'future' (合约) 或 'spot' (现货)
    MARKET_TYPE = "future"
    
    # 24h成交额过滤 (默认只监控 > 50万U 的币种)
    MIN_24H_QUOTE_VOLUME = 500000 
...
### 🏆 A+ 级信号：全球协同看涨 (Global Sync Bullish)
> **注意**: 仅保留看涨方向信号。


**触发条件**:
- ✅ 所有有效平台净流入/流出方向一致
- ✅ 所有有效平台买卖比 > 1.15 (看涨) 或 < 0.85 (看跌)
- ✅ 5m/1h 趋势与信号方向一致 (多周期共振)

**信号含义**:
主力资金在全球范围内同步行动，市场情绪高度一致。

---

### 💎 A 级信号：稳步上涨 (Steady Growth)
**触发条件**:
- ✅ 15m 周期均线多头排列 (MA20 > MA60)
- ✅ 走势温和稳定 (无暴涨暴跌)
- ✅ 趋势斜率持续向上

**信号含义**:
主力资金在进行有计划的稳健推升，趋势可持续性强，适合顺势入场。

**日志示例**:
```
💎 [ETH/USDT] 稳步上涨趋势确认 (MA多头排列, 斜率+0.08%)
```
...
### 使用方法
```bash
# 回测 ETH/USDT 过去 3 天的数据
python src/backtest.py ETH/USDT 3
```

### 回测报告示例
```
==================================================
📊 Backtest Results for ETH/USDT (3 days)
==================================================
Total Trades: 5
Win Rate:     60.00% (3W / 2L)
Total PnL:    $1,250.00
Final Bal:    $11,250.00 (12.50%)
--------------------------------------------------
Latest 5 Trades:
2025-12-28 10:00:00 LONG PnL: $500.00 (TP)
